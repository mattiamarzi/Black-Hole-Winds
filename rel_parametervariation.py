    ...
    for i0 in range(len(lambda_edd)):
        ...
        for i1 in range(len(x0)):
            ...
            for i2 in range(len(xi0)):
                vout=v0[i1]
                n0r0=gamma(vout)*Psi(vout)/((Upsilon(vout))**2)*lambda_edd[i0]*4.39*1E32/(k*(10**xi0[i2])*x0[i1])
                ...
                if alpha[i]==0:
                    ...
                    nh=np.linspace(0,1E22,50) #let's redefine nh to remove previous modifications
                    nh_limit=n0r0	
                    nh_ref=0 
            ...
            plt.subplot(3,len(x0),counter+len(x0)*2)
            for i2 in range(len(xi0)):
                if alpha[i]==0:
            ...
            for i2 in range(len(xi0)):
                xi_r=np.zeros(len(nh))
                for i3 in range(len(nh)):
                    if alpha[i]==0:
                        xi_r[i3]=np.log10(Psi(v_r[i3])/Psi(vout)*gamma(v_r[i3])/gamma(vout)*(Upsilon(vout)/Upsilon(v_r[i3]))**2*10**xi0[i2]*(ratio[i2,i3]+1)**(-2))
                    elif alpha[i]==1:
                        xi_r[i3]=np.log10(Psi(v_r[i3])/Psi(vout)*gamma(v_r[i3])/gamma(vout)*(Upsilon(vout)/Upsilon(v_r[i3]))**2*10**xi0[i2]*(np.exp(ratio[i2,i3]))**(-1))
                    elif alpha[i]==2:
                        xi_r[i3]=np.log10(Psi(v_r[i3])/Psi(vout)*gamma(v_r[i3])/gamma(vout)*(Upsilon(vout)/Upsilon(v_r[i3]))**2*10**xi0[i2]*(1/(1-ratio[i2,i3]))**(0))
        ...
        plt.savefig('Relativistic Parameters Variation α={:1.0f}, $λ_{{Edd}}$={:3.2f}.png'.format(alpha[i], lambda_edd[i0]))
